...
export const toolType = 'rectangleAi';

// /////// BEGIN ACTIVE TOOL ///////
export function createNewMeasurement (annoData) {
...
export function addNewMeasurement (aiData) {
  const element = aiData.element;

  const measurementData = createNewMeasurement(aiData.data);

  // Associate this data with this imageId so we can render it and manipulate it
  cs.cornerstoneTools.addToolState(element, toolType, measurementData);
}

// /////// END ACTIVE TOOL ///////

export function pointNearTool (element, data, coords) {
  const startCanvas = cs.cornerstone.pixelToCanvas(element, data.handles.start);
  const endCanvas = cs.cornerstone.pixelToCanvas(element, data.handles.end);
  ...
  const distanceToPoint = cs.cornerstoneMath.rect.distanceToPoint(rect, coords);
...
export function onImageRendered (e) {
  const eventData = e.detail;

  // If we have no toolData for this element, return immediately as there is nothing to do
  const toolData = cs.cornerstoneTools.getToolState(e.currentTarget, toolType);
  ...
  const lineWidth = cs.cornerstoneTools.toolStyle.getToolWidth();
  const config = cs.cornerstoneTools.rectangleAi.getConfiguration();
  const context = eventData.canvasContext.canvas.getContext('2d');
  const seriesModule = cs.cornerstone.metaData.get('generalSeriesModule', image.imageId);
  let modality;
  ...
  for (let i = 0; i < toolData.data.length; i++) {
    ...
    const color = cs.cornerstoneTools.toolColors.getColorIfActive(data.active);

    // Convert Image coordinates to Canvas coordinates given the element
    const handleStartCanvas = cs.cornerstone.pixelToCanvas(element, data.handles.start);
    const handleEndCanvas = cs.cornerstone.pixelToCanvas(element, data.handles.end);
    ...
    if (config && config.drawHandlesOnHover) {
      ...
      if (data.active === true) {
        cs.cornerstoneTools.drawHandles(context, eventData, data.handles, color);
      } else {
        ...
        cs.cornerstoneTools.drawHandles(context, eventData, data.handles, color, handleOptions);
      }
    } else {
    ...
    const textCoords = cs.cornerstone.pixelToCanvas(element, data.handles.textBox);
    ...
    const boundingBox = cs.cornerstoneTools.drawTextBox(context, textLines, textCoords.x,
    ...
    if (data.handles.textBox.hasMoved) {
      ...
      link.start = cs.cornerstoneMath.point.findClosestPoint(ellipsePoints, link.end);
      ...
      link.end = cs.cornerstoneMath.point.findClosestPoint(boundingBoxPoints, link.start);
